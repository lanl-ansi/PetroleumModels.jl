...
for n in nws
         ...
         B =  (pm.data["rho"] * pm.data["gravitational_acceleration"]  * sum(pm.ref[:nw][n][:pump][i]["electricity_price"] * JuMP.value(pm.var[:nw][n][:q][i]) * pm.data["baseQ"] * JuMP.value((pm.var[:nw][n][:H][pump["t_junction"]]) -
         JuMP.value(pm.var[:nw][n][:H][pump["f_junction"]])) * pm.data["baseH"] / (0.966 * 0.95 * JuMP.value(pm.var[:nw][n][:eta][i])) / 1000 for (i, pump) in pm.ref[:nw][n][:pump]) )
...
for n in 1:nws
     for i in sort(collect(ids(pm, :junction)))
         push!(H_values, (JuMP.value(pm.var[:nw][n][:H][i])) * pm.data["baseH"])
     end
     ...
     for i in sort(collect(ids(pm, :producer)))
          push!(qg_value, JuMP.value(pm.var[:nw][n][:qg][i]) * pm.data["baseQ"] * 3600)
     end
    ...
    # println("producer=", qg_value)
     ...
     for i in sort(collect(ids(pm, :consumer)))
          push!(ql_value, JuMP.value(pm.var[:nw][n][:ql][i]) * pm.data["baseQ"] * 3600)
     end
 ...
 # println(keys(q_min), q_min)
    ...
    writedlm("all_values_$n.csv", dual_values, ",")
